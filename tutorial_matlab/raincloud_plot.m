...
% plot_means                - [0|1] whether or not the means and SDs should be plotted in addition
%
% ---------------------------- OUTPUT ----------------------------
...
% ------------------------ EXAMPLE USAGE -------------------------
%
% [handles, kernel_bandwidths, dot_locations] = raincloud_plot(myData, 'box_on', 1, 'color', [0.5 0.5 0.5])
%
...
addOptional(p, 'plot_means', 0, @isnumeric)
...
plot_means = p.Results.plot_means;
...
% plot means if selected

if plot_means
	plot(mean(data),1:size(data,2),'ko','markerfacecolor','k')
	
	for data_part = 1:size(data,2)
		
		plot([mean(data(:,data_part))-std(data(:,data_part)); mean(data(:,data_part))+std(data(:,data_part))],[data_part data_part],'k','linewidth',1.5);
